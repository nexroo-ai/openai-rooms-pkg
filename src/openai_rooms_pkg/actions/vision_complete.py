# FILE: src/openai_rooms_pkg/actions/vision_complete.py
...
class ActionInput(BaseModel):
    messages: List[Dict[str, Any]] = Field(..., description="Chat messages (can include image_url parts)")
    images: Optional[List[str]] = Field(default=None, description="Optional extra image URLs or data URIs")
...
def _attach_images_to_messages(messages: List[Dict[str, Any]], images: List[str]) -> List[Dict[str, Any]]:
    """Attach image_url parts to the last user message, preserving existing multimodal content."""
    if not images:
        return messages
    ...
    for i in range(len(messages) - 1, -1, -1):
        if messages[i].get("role") == "user":
            ...
            break
    if idx is None:
        # If no user message, append a new one
...
def vision_complete(config: CustomAddonConfig, action_input: ActionInput) -> ActionResponse:
    logger.debug("vision_complete: starting with inputs")
    logger.debug(
        f"messages count={len(action_input.messages)}, images={len(action_input.images or [])}, "
        f"model={action_input.model or config.model_default}"
    )

    # Validate that images are present if messages are not already multimodal
    if not action_input.images:
        # check if any message already has image_url parts
        has_image = False
        for m in action_input.messages:
            c = m.get("content")
            if isinstance(c, list) and any((p.get("type") == "image_url") for p in c if isinstance(p, dict)):
                has_image = True
                break
        if not has_image:
            # No images and not multimodal content
            ...
            return ActionResponse(
                ...
                message="400: images are required or messages must already contain image_url parts",
                code=400,
            )
    ...
    try:
        ...
        logger.debug(f"vision_complete: headers={redacted_headers}")

        base_url = (config.api_base or "https://api.openai.com/v1").rstrip("/")
        url = f"{base_url}/chat/completions"

        messages = action_input.messages
        if action_input.images:
            messages = _attach_images_to_messages(messages, action_input.images)
        ...
        logger.debug(f"vision_complete: POST {url}")
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.debug(f"vision_complete: status_code={status}")
                if status >= 400:
                ...
                last_err_msg = str(e)
                logger.debug(f"vision_complete: request error on attempt {attempt+1}: {last_err_msg}")
                if attempt < max_attempts - 1:
                    time.sleep(0.8 * (2 ** attempt))
                    continue
                output = ActionOutput(result="", raw=None, model=payload.get("model") or "", usage=None)
        ...
        logger.debug(f"vision_complete: exception {e}")
        output = ActionOutput(result="", raw=None, model=action_input.model or (config.model_default or ""), usage=None)
        return ActionResponse(
            output=output,
            tokens=TokensSchema(stepAmount=0, totalCurrentAmount=0),
